...
pub fn add_new_tree(ctx: Context<AddTree>) -> Result<()> {
    msg!("-- Initializing merkle tree");
    require!(ctx.accounts.faucet.authority == ctx.accounts.authority.key(), OnnyxError::InvalidAuth);
    require!(ctx.accounts.merkle_tree.data.borrow().len() == REQUIRED_TREE_ACCOUNT_SIZE, OnnyxError::InvalidTree);
    let signer_seeds: &[&[&[u8]]] = &[&[
