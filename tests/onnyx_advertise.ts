...
describe("onnyx_advertise", () => {
  ...
  const conversions = [{click: [new anchor.BN(5), new anchor.BN(1)]}];
  ...
  describe.only("CAMPAIGN IXs", () => {
    it("create campaign", async () => {
      const ix = await createCampaignIx(program, advertiser.publicKey, campaignName, conversions, audiances);
 ...
 * - add sol transfers for updating conversion parameters on the campaign
 */